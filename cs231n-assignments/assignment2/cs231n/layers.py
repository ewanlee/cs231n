...
def affine_forward(x, w, b):
  ...
  #############################################################################
  num_input = x.shape[0]
  x_reshape = x.reshape((num_input, -1))
...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    norm_x = (x - sample_mean) / (sample_var + eps) ** 0.5
    ...
    # the out variable.                                                         #
    #############################################################################
    norm_x = (x - running_mean) / (running_var + eps) ** 0.5
    out = norm_x * gamma + beta
...
def batchnorm_backward(dout, cache):
  ...
  inv_std = 1 / (var + eps) ** 0.5

  N, D = x.shape

  dbeta = np.sum(dout, axis=0)
  dgamma = np.sum(((x - mean) / (var + eps) ** 0.5) * dout, axis=0)

  dxhat = dout * gamma
  dvar = np.sum(dxhat * (x - mean), axis=0) * (-0.5 * np.power(var + eps, -1.5))
  du1 = np.sum(dxhat * (-1 / (var + eps) ** 0.5), axis=0)
  du2 = dvar * (-2) * np.sum(x - mean, axis=0) / N
  du = du1 + du2
  dx = dxhat * (1 / (var + eps) ** 0.5) + dvar * 2 * (x - mean) / N + du / N
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
...
def batchnorm_backward_alt(dout, cache):
  ...
  dx = (1. / N) * inv_std * (N * dxhat - np.sum(dxhat, axis=0)\
...
def conv_forward_naive(x, w, b, conv_param):
  ...
  padded_x = np.lib.pad(
    x, ((0, 0), (0, 0), (1, 1), (1, 1)), mode='constant', constant_values=0)

  for n in xrange(N):
    for f in xrange(num_filters):
      for h in xrange(H_prime):
      	for w in xrange(W_prime):
        	out[n][f][h][w] = \
        	np.sum(padded_x[n, :, \
        		h*stride:h*stride+HH, \
                w*stride:w*stride+WW] * weigh[f]) + b[f]
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  cache = (x, weigh, b, conv_param)
  return out, cache
